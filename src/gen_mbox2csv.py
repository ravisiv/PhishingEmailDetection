    ...
    for each_url in all_urls:
        ...
        try:
            ...
            topdomains_df = pd.concat([topdomains_df, pd.DataFrame(
                {"topdomain": [tdomain]})], ignore_index=True)
        ...
        if suffix != '':
            try:
                ...
                suffix_df = pd.concat([suffix_df, pd.DataFrame(
                    {"suffix": [suffix]})], ignore_index=True)
    ...
    for each_header in all_headers:
        ...
        try:
            ...
            headers_df = pd.concat([headers_df, pd.DataFrame(
                {"header": [strip_header]})], ignore_index=True)
...
data_path = scratch_dir + "mboxfiles"
...
for folder in sub_folders:
    ...
    for file in files:
        ...
        f_realpath = f"{folder}/{file}"
        mbox_obj = mailbox.mbox(f"{folder}/{file}")
...
urls_df.to_csv(master_urls)
topdomains_df.to_csv(master_topdomains)
subdomains_df.to_csv(master_subdomains)
suffix_df.to_csv(master_suffix)
headers_df.to_csv(master_headers)


metadata_mbox_df = pd.DataFrame.from_dict(metadata_mbox_all)
metadata_mbox_df = pd.DataFrame.from_dict(metadata_mbox_all)
email_df = pd.concat([metadata_mbox_df, metadata_df],
                     ignore_index=True, sort=False)
email_df.to_csv(scratch_dir + "masterdata/allemails.csv")
